...
module.exports = {
    'planAB': function planAB(inbound, tMax, tDest){
        // A->B
        ...
        return [acc_B, tRemain];
    },
    'planBC': function planBC(location, tRemain, outbound, tMax, tDest, tDelta){
        // B->C
        // calculate time offset for duration at B
        let tDestOffset = getOffset(tDest, tDelta);

        // create result matrix
        let nl = tRemain.shape[0];
        let nt = tRemain.shape[1];
        ...
        let jl = location; // selected start location index
