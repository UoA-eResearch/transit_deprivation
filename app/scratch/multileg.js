...
let tRemain = validApply(inbound, null, (a) => {return tMax})

// subtract travel time to destination and time at destination (clip negative values to 0)
tRemain = validApply(tRemain, inbound, (a, b) => {return Math.max(0, a - b - tDest)});
...
for (let jt = 0; jt < nt; jt++){
    ...
    if (tRemainJ > EPS){
        ...
        if (BCj.shape[1] > 0){
            ...
            let tRemainBCj = validApply(BCj, null, (a) => {return tRemainJ});

            // subtract travel time from B to C
            tRemainBCj = validApply(BCj, tRemainBCj, (a, b) => {return Math.max(0, b - a)});
