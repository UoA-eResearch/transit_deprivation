...
```{r}
routes <- otp_plan(otpcon, fromPlace = c(174.71354, -36.88028), toPlace = c(174.73435, -36.88366), mode=c("TRANSIT", "WALK")) # lon lat pairs
tmap_mode("view")          # Set tmap to interactive viewing
qtm(sf::st_zm(routes))      # Plot the route on a map
```
...
routes = otp_plan(otpcon, fromPlace=fromPlace, toPlace=toPlace, mode=c("TRANSIT", "WALK"), fromID=fromID, toID=toID,
                  date_time=queryTime, get_geometry=FALSE, ncores=7)
